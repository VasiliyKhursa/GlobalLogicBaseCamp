...
static long dev_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
   ...
   switch( cmd ) {
      case IOCTL_GET_STRING:
         if ( copy_to_user( (void*)arg, hello_str, strlen(hello_str) ) )
             return -EFAULT;
         break;
      default:
         return -ENOTTY;
...
static int __init dev_init( void ) {
   ...
   printk( KERN_INFO "=== Start init the char device\n" );

   ret = register_chrdev( HELLO_MAJOR, HELLO_MODNAME, &hello_fops );
